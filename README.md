...
### Caching
On start up, the program parses all the GTFS file afresh, with no caching of any kind. This is computationally wasteful, but since it only takes a few seconds, I wasn't motivated to implement some kind of serialization. However, it shouldn't be a massive undertaking and would improve performance. This would be desired if later, more resourcing taxing features are added, such as live data.
